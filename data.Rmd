...
df <- select( df, c(PositionsList, Team, Cost, TotalPoints, MinutesPlayed, GoalsScored, GoalsScoredPoints, Assists, CleanSheets, CleanSheetPoints, GoalsConceded, GoalsConcededPoints, PenaltiesMissed, OwnGoals, YellowCards, RedCards, Bonus, SelectedByPercent, LastSeasonPoints ))

df <- df %>% 
   mutate(Cost = Cost / 1000000)            #convert cost to millions
   filter(df, PositionsList != "GLK")       #not analyzing goalkeepers
...
# We only want to consider players with at least 100 points
d <- filter(df, TotalPoints > 100)
...
# Each player gets 3 points per assists
d <- d %>% 
   mutate(AssistPoints = Assists * 3)

#Dealing with minutes played points
