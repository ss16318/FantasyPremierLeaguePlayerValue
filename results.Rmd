...
What we see in the graph is reflective of reality, that the soccer players on the bottom of the bottom teams are mostly concentrated around a mean of £4-5 million, but as you climb up the board, the mean shifts to between £5-6 million, and the tails get longer and longer. In fact, the top performing teams appear to have multi-modal distributions in which subsets of their are very expensive.
Using the previous analysis that the the best soccer players cost the most, this follows the claim that the best performing soccer teams have the best performing soccer players.
...
Qunatifying this, we found that about 75% of players have Costs which vary less than +/- £200,000 over the course of the entire season. As such, we decided to graph the “Costs” of players over the 2021 season whose price went up or down by £300,000 or more. 
...
Another angle we approached the project question from was trying to find a pattern with Selected By Percentage because if we find soccer players that aren't picked by many FPL users, we could benefit more from their points. 

From our research, we found that Selected By Percentage is a driver of player cost. For example, if a lot of FPL users choose a soccer player, his demand is high, so his Cost increases. Therefore, we decided to plot a scatter plot of total points against Selected By Percentage to explore any potential trends. However, as can be seen in the graph, there is no obvious pattern in the data. 
...
However, when plotting the weekly change in cost of the top 7 most expensive players in FPL during 2021 next to the weekly Selected By Percentage of these soccer players by FPL users, we see evidence of correlations between the two. For example, in Week 23 we see the Cost and Selected By Percentage of dip, while we see a peak for Ronaldo. With this in mind, one avenue of further work could look into the correlation between total points and Selected By Percentage because we know there is a positive correlation between total points and cost and there may be another positive correlation between cost and Selected By Percentage. Therefore, an indicator for player value could be a ratio between Selected By Percentage and Total Points 
...
g1 <- ggplot(pc_graph_2021_beg, aes(x=Week, y=SelectedByPercent, group = Surname, colour=factor(Surname))) +
  ...
  labs(colour = "Player") +
  ylab("Selected By Percentage") + xlab("Game Week") +
  theme_light() +
  theme(axis.text.x = element_text(size = 7, angle=45)) +
